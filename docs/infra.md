...
| Workers Builds | git integration on the Worker above, production branch `main` | — | 6 |
...
### 7. Connect Workers Builds (Phase 6)

Push-to-deploy. Dashboard-only — there is no `wrangler` command and nothing in
`wrangler.jsonc` records it, which is exactly why it is written down here.

Dashboard → Workers & Pages → `hololive-ocg-wiki-tskrlabs-com` → Settings → **Build** →
Connect repository → `tskrlabs/hololive-ocg-wiki`.

Cloudflare supports connecting an **existing** Worker, so the git integration goes on top
of the Worker Phase 5 deployed by hand, with no rework. The dashboard name must match
`name` in `wrangler.jsonc` — which is why that name was settled in Phase 5 rather than
here.

| Setting | Value | Why |
|---|---|---|
| Git branch (production) | `main` | `develop` is the integration line; merging to `main` is the release action, matching how the branches are already used |
| Build command | `npm run generate --workspace @holo/web` | `assets.directory` is `apps/web/.output/public`, which is gitignored — without this the Worker deploys an empty site |
| Deploy command | `npx wrangler deploy --config apps/api/wrangler.jsonc` | `--config` because the command runs from the repo root, not `apps/api` |
| Root directory | `/` (blank) | npm workspaces: the install must happen at the root or `@holo/schema` does not link into `apps/web` and `apps/api` |
| Builds for non-production branches | **off** | a preview version uploads with the *same* bindings, so a preview URL is a second live window onto production D1 — D12 deferred the preview environment deliberately |
| Build caching | **off** | builds are rare (merges to `main` only) and a cache that restores `node_modules` is the shape of a stale-generated-artifact bug, which is what this repo is built to prevent |
| API token | Cloudflare's auto-generated one | see below |

**Node is pinned to 24** by `.node-version` at the repo root. The build image defaults to
22.16 and Cloudflare bumps it without notice; unpinned, the deploy would run a different
major than every phase was verified against. 24 also ships npm 11, which is what makes the
root `allowScripts` block meaningful.

**The build token is the one account-wide credential here, deliberately.** The R2 and D1
tokens above are narrowly scoped because they sit in `pipeline/.env` on a laptop and are
handed to a CLI an agent may drive (D4) — that is the blast radius those scopes bound. The
build token never leaves Cloudflare: nothing in this repo can read it, and the only thing
that invokes it is a push to `main`. Minting a custom one would add a rotation chore and a
"why did the build start failing" mode without narrowing anything we control.

**Manual and build-triggered deploys both produce versions**, and whichever is promoted
last is active. So a manual `wrangler deploy` after this point overrides the last pushed
build until the next push — fine as an emergency path, surprising if forgotten.

A failed build promotes nothing, so the live site is unaffected. The bad outcome of a
broken build is "no deploy", not "broken deploy".

## Local development — no credentials at all

D12's requirement is that a fresh clone runs with zero Cloudflare credentials. `make dev`
does all of this; the underlying commands are:
...
apps/api/scripts/seed-local-r2.sh     # the R2 half — filter-options, info, status
```
...
**Both stores are seeded from committed files**, so this needs `wrangler` and nothing
else. `fixtures/artifacts/` (64 KB — seven `filter-options/{locale}.json` plus
`status.json`) is generated by `make generate` and its staleness is caught by
`make check`; before Phase 6 it was built on every `make dev` by a Python script that
imports pydantic, which put `uv sync` and a venv between a fresh clone and a working site.

Verified from a scratch clone with `uv` unavailable, each phase — see `progress.md`.
